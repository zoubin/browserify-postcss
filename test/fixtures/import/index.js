...
test('import', function (t) {
  ...
  t.ok(
    ['rgba(0, 0, 0, 0)', 'transparent'].indexOf(
      style1['background-color'] || style1.backgroundColor
    ) !== -1,
    'default background-color should be transparent white or transparent'
  )
  t.equal(style1.color, 'rgb(0, 0, 0)', 'default color should be black')

  var link = require('./index.scss')
  link.onload = function (e) {
    window.clearTimeout(timer)
    var style2 = window.getComputedStyle(el, null)
    t.equal(
      style2['background-color'] || style2.backgroundColor,
      'rgb(128, 0, 128)',
      'imported background-color should be applied'
    )
